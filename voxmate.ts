...
export type UserInputBase = { sequence: number; isInterrupt: boolean; origin: number }
...
export interface IGPSLocation extends IGeoLocation {
    accuracy: number;
    speed?: number | null;    // ground speed in m/s, null when the platform cannot supply it
    altitude?: number | null; // metres above sea level, null when unavailable
}

export interface ICompassReading {
    azimuth: number;  // 0..360 degrees clockwise from magnetic north (device pointing direction)
    accuracy: number; // sensor accuracy hint: -1 unknown, 0 unreliable, 1 low, 2 medium, 3 high
}

export interface IHeadingReading {
    heading?: number; // 0..360 degrees direction of travel, absent (== null) when stationary/unknown
    speed: number;    // ground speed in m/s backing the heading estimate
}
...
export type VoxletPermission =
    "microphone" | "camera" | "location" | "network" | "unrestricted-network" | "notifications" |
    "clipboard" | "apps" | "storage" | "ai" | "calls" | "contacts";
...
// "unavailable": the agent builder is not enabled on this engine — feature-detect on this value
// rather than probing. "building": accepted, build in progress. "ready": built and deployed; `ident`
// names the resulting voxlet. "failed": build failed; `error` carries the compile output.
export type AgentBuildStatus = "unavailable" | "building" | "ready" | "failed";

export interface IAgentBuildResult {
...
export interface IVoxmateStatic {
    ...
    permissions: {
        queryGrants(): Partial<Record<VoxletPermission, boolean>>;
    };
    ...
    location: {
        ...
        watch(handler: (location: IGPSLocation) => void): DeregisterFunction
    };

    compass: {
        watch(handler: (reading: ICompassReading) => void): DeregisterFunction
        watchHeading(handler: (reading: IHeadingReading) => void): DeregisterFunction
    };
    ...
    system: {
        ...
        grants: {
            recordGrant(ident: string, permission: VoxletPermission, granted: boolean): IVoxmatePromise<boolean>;
        }
